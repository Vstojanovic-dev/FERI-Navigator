...
-- navigation_locations for G3 destinations are now generated by
-- 006_admin_navigation_graph.sql from the admin snapshot.
-- Do not seed them again here because the admin export may already have
-- remapped destination spaces to canonical nodes, which would conflict with
-- the legacy node-based inserts below.
...
resolved AS (
    SELECT
        ...
        source_space.code AS space_code,
